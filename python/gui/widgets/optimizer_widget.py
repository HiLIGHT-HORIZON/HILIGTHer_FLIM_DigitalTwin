...
class OptimizationWorker(QThread):
    """Worker thread to run gate optimization without freezing the UI."""
    ...
    def run(self):
        try:
            ...
            print(f"Optimization Thread Error: {exc}")
            self.finished.emit((None, 0.0, {}))


class OptimizerWidget(QWidget):
    """Dialog widget for gate optimization."""

    gates_optimized = pyqtSignal(list)
    ...
    def __init__(self, engine):
        ...
        self.gate_plot = pg.PlotWidget(title="Optimized Gates and IRF")
        ...
        self.metric_plot.setLabel("left", "F-Value (Precision)")
    ...
    def run_optimization(self):
        ...
        self.worker.progress.connect(lambda j: self.txt_output.append(f"Current score: {j:.6f}"))
    ...
    def on_optimization_finished(self, results):
        ...
        algorithm = info.get("algorithm", "direct_slsqp") if isinstance(info, dict) else "direct_slsqp"
        out = f"\nOptimization complete.\nAlgorithm: {algorithm}\nObjective J: {j_val:.6f}\n\nOptimal edges (ns):\n"
        out += ", ".join(f"{edge:.3f}" for edge in edges)
    ...
    def export_results(self):
        if self.best_edges:
            ...
            self.txt_output.append("\nOptimized gates exported successfully.")
