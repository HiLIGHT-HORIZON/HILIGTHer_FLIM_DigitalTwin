...
%         
        while (rp<N); %SCAN DATA BLOCK
            ...
            [seg_macrotime seg_routing seg_mark seg_gap seg_mtov seg_invalid seg_utime seg_checker remainder seg_overflow] = seg_data_extraction2(seg_A);
           ...
           if mark(ip)==1
               ...
               %
                ...
                if ~invalid(ip)& ~(x>x_pxl | y>y_pxl)   
                    t = ceil((utime(ip)+1)/((tac_res+delay)/n_bins)); % identify time bin
                    if bTest
                        img((mi-1)*16+routing(ip)+1,t) = img((mi-1)*16+routing(ip)+1,t) + 1;                      
                    else
